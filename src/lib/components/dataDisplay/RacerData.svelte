  ...
  $: {
    if(racer.lapTimes.length > 0){
        if(lastElement instanceof HTMLElement){
            lastElement.scrollIntoView({block: 'nearest', inline: 'start'});
...
<div>
    <Table shadow striped={true} divClass="dark:bg-gray-600 max-h-[70svh] overflow-y-scroll">
      <caption
      ...
      <TableBody tableBodyClass="divide-y-2 dark:divide-gray-500 divide-gray-400">
        {#each racer.lapTimes as times, index}
          <TableBodyRow>
            ...
            {:else}
              <TableBodyCell
                >{times - racer.lapTimes[index - 1]}</TableBodyCell
              >
            {/if}
